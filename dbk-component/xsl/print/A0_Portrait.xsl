...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:fox="http://xmlgraphics.apache.org/fop/extensions" xmlns:svg="http://www.w3.org/2000/svg" exclude-result-prefixes="fo">
  ...
  <xsl:template name="dbk_titleBk">
    ...
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <fo:block-container width="40.74999999999999cm" height="46.44cm" top="0cm" left="0cm" xsl:use-attribute-sets="column-block-border">
  ...
  <xsl:template name="dbk_specialBk">
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <fo:block-container width="40.749cm" height="46.44cm" top="0cm" left="40.949cm" xsl:use-attribute-sets="column-block-border">
        <!-- Bijzonderheden.-->
        ...
        <fo:block margin-top="0.2cm" margin-left="0.2cm" xsl:use-attribute-sets="default-font">
          <fo:table table-layout="fixed" width="40.349cm">
            <fo:table-column column-width="8.876cm" border-width="thin" border-right-style="solid"></fo:table-column>
            <fo:table-column column-width="6.052cm" border-width="thin" border-right-style="solid"></fo:table-column>
            <fo:table-column column-width="5.245cm" border-width="thin" border-right-style="solid"></fo:table-column>
            <fo:table-column column-width="5.245cm" border-width="thin" border-right-style="solid"></fo:table-column>
            <fo:table-column column-width="3.631cm" border-width="thin" border-right-style="solid"></fo:table-column>
            <fo:table-column column-width="1.613cm"></fo:table-column>
            <fo:table-column column-width="1.613cm"></fo:table-column>
            <fo:table-column column-width="1.613cm"></fo:table-column>
            <fo:table-column column-width="1.613cm"></fo:table-column>
            <fo:table-column column-width="1.613cm"></fo:table-column>
      ...
      <xsl:for-each select="foto">
        <xsl:if test="*">
          <xsl:if test="position()=1">
            <xsl:if test=".!='null'">
              <fo:block-container width="40.749cm" height="34.83cm" top="46.64cm" left="0cm" xsl:use-attribute-sets="column-block-border">
      ...
      <xsl:for-each select="foto">
        <xsl:if test="*">
          <xsl:if test="position()=2">
            <xsl:if test=".!='null'">
              <fo:block-container width="40.749cm" height="34.83cm" top="46.64cm" left="40.949cm" xsl:use-attribute-sets="column-block-border">
                <fo:block margin="0.05cm">
                  <xsl:variable name="photoUrl" select="URL"></xsl:variable>
                  <fo:external-graphic src="{$photoUrl}" width="100%" height="100%" content-width="scale-to-fit"></fo:external-graphic>
                </fo:block>
  ...
  <xsl:template name="dbk_dangerousSubstBk">
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <fo:block-container width="81.699cm" height="34.83cm" top="81.67cm" left="0cm" xsl:use-attribute-sets="column-block-border">
