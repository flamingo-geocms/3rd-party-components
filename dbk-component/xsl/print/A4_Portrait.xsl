      ...
      <xsl:for-each select="foto">
        <xsl:if test="*">
          <xsl:if test="position()=1">
            <xsl:if test=".!='null'">
              <fo:block-container width="9.25cm" height="8.13cm" top="10.94cm" left="0cm" xsl:use-attribute-sets="column-block-border">
      ...
      <xsl:for-each select="foto">
        <xsl:if test="*">
          <xsl:if test="position()=2">
            <xsl:if test=".!='null'">
              <fo:block-container width="9.25cm" height="8.13cm" top="10.94cm" left="9.35cm" xsl:use-attribute-sets="column-block-border">
                <fo:block margin="0.05cm">
                  <xsl:variable name="photoUrl" select="URL"></xsl:variable>
                  <fo:external-graphic src="{$photoUrl}" width="100%" height="100%" content-width="scale-to-fit"></fo:external-graphic>
                </fo:block>
