...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:fox="http://xmlgraphics.apache.org/fop/extensions" xmlns:svg="http://www.w3.org/2000/svg" exclude-result-prefixes="fo">
  ...
  <xsl:template name="dbk_titleBk">
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <fo:block-container width="6.15cm" height="6.072cm" top="0cm" left="0cm" xsl:use-attribute-sets="column-block-border">
        <fo:block margin-top="0.1cm" margin-left="0.1cm" xsl:use-attribute-sets="title-font">Digitale bereikbaarheidskaart Utrecht</fo:block>
  ...
  <xsl:template name="dbk_specialBk">
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <fo:block-container width="6.15cm" height="6.072cm" top="0cm" left="6.25cm" xsl:use-attribute-sets="column-block-border">
        <!-- Bijzonderheden.-->
        ...
        <fo:block margin-top="0.2cm" margin-left="0.1cm" xsl:use-attribute-sets="default-font">
          <fo:table table-layout="fixed" width="5.95cm">
            ...
            <fo:table-column column-width="1.19cm" border-width="thin" border-right-style="solid"></fo:table-column>
            <fo:table-column column-width="1.19cm" border-width="thin" border-right-style="solid"></fo:table-column>
            <fo:table-column column-width="0.595cm"></fo:table-column>
            <fo:table-body>
              <fo:table-row>
                <fo:table-cell margin-left="0.05cm">
    ...
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <xsl:for-each select="foto">
        <xsl:if test="position()=1">
          <fo:block-container width="6.15cm" height="6.072cm" top="6.172cm" left="0cm" xsl:use-attribute-sets="column-block-border">
    ...
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <xsl:for-each select="foto">
        <xsl:if test="position()=2">
          <fo:block-container width="6.15cm" height="6.072cm" top="6.172cm" left="6.25cm" xsl:use-attribute-sets="column-block-border">
            <fo:block margin="0.05cm">
              <xsl:variable name="photoUrl" select="URL"></xsl:variable>
              <fo:external-graphic src="{$photoUrl}" width="100%" height="100%" content-width="scale-to-fit"></fo:external-graphic>
            </fo:block>
  ...
  <xsl:template name="dbk_dangerousSubstBk">
    <xsl:for-each select="extra/info[@classname='viewer.components.Dbk']/root">
      <fo:block-container width="12.4cm" height="6.072cm" top="12.344cm" left="0cm" xsl:use-attribute-sets="column-block-border">
